...
class Vector(object):
    ...
    def __init__(self, vect):
        self.ndim = len(vect)
        self.vect = np.array(vect, dtype=np.float).reshape(-1, 1)
...
class VectorContainer(object):
    ...
    def __setattr__(self, name, value):
        assert issubclass(value.__class__, Vector)
...
class Shape(object):
    def __init__(self, ndim):
        self.ndim = ndim
        self.vects = VectorContainer()

    def rotate(self, axis, angle, degrees=True):
        [p.rotate(axis, angle, degrees) for p in self.vects]

    def scale(self, vect):
        [p.scale(vect) for p in self.vects]

    def translate(self, vect):
        [p.translate(vect) for p in self.vects]
...
class Parallelogram(Shape2D):
    ...
    def bounding_box(self):
        ...
        pt_list = np.array([xref,
                            ...
                            (xref + self.vects.vec1()
                             + self.vects.vec2())])
        self.vects.pt_min = Point(pt_list.min(axis=0))
        self.vects.pt_max = Point(pt_list.max(axis=0))
...
class Parallelepiped(Shape3D):
    ...
    def project(self, x):
        ...
        """
        x = np.atleast_2d(np.array(x) - self.vects.xref())
        den = np.linalg.det(self.mat)
        num_a = np.apply_along_axis(
            lambda m: np.linalg.det(
                np.vstack([m, self.mat[:, 1], self.mat[:, 2]])), 1, x)
        num_b = np.apply_along_axis(
            lambda m: np.linalg.det(
...
class Disc(Shape2D):
	...
	if (len(radius)==1):
	        self.vects.radius = Vector((np.append(radius, 0.0).reshape(-1,1)))
	else :
    ...
    def bounding_box(self):
        """Set bounding box for shape"""
	self.vects.pt_min = Point(self.vects.center() - np.linalg.norm(self.vects.center()))
        self.vects.pt_max = Point(self.vects.center() + np.linalg.norm(self.vects.center()))
    ...
    def area(self):
        """Get disc area"""
	return np.pi * (np.linalg.norm(self.vects.center()))**2
    volume = area
	...
	return(self.distance(x) <= np.linalg.norm(self.vects.radius()))
...
class Sphere(Shape3D):
	...
	radius = np.ravel(radius)
        if (len(radius)==1):
                self.vects.radius = Vector((np.append(radius, [0.0, 0.0]).reshape(-1,1)))
        else :
                self.vects.radius = Vector(radius)
        self.check()
        self.bounding_box()
    ...
    def bounding_box(self):
        """Set bounding box for shape"""
	self.vects.pt_min = Point(self.vects.center() - np.linalg.norm(self.vects.radius()))
        self.vects.pt_max = Point(self.vects.center() + np.linalg.norm(self.vects.radius()))
    ...
    def volume(self):
        """Get sphere volume"""
	return (4./3. * np.pi * np.linalg.norm(self.vects.radius())**3)
...
class Cylinder(Shape3D):
    ...
    def bounding_box(self):
        ...
        def min_max_cyl(axis):
            ...
            vec_axis = self.vects.ax_vec()[axis]
            length = np.linalg.norm(self.vects.ax_vec())
    ...
    @property
    def args(self):
        return [self.vects.xref(), self.radius, self.vects.ax_vec()]
    ...
    def project(self, x):
        ...
        Get cylinder axis coord (from xref) and distance to axis
        (normalized by ||ax_vec|| and radius)
        x = axis * ax_vec/||ax_vec|| + r * dist_ax/||dist_ax||
        See http://www.flipcode.com/archives/Fast_Point-In-Cylinder_Test.shtml
        """
        x -= self.vects.xref()
        dot = np.dot(x, self.vects.ax_vec())
        axis_coord = dot / self.vects.ax_vec.length**2
        r_coord = ((np.linalg.norm(x, axis=-1)**2 - axis_coord*dot)
                   / self.radius**2)
